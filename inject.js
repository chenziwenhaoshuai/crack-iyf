(function() {
    ...
    if (IS_TOP) {
        ...
        function createMasterPanel() {
            ...
            box.style.cssText = `
                ...
                border: 2px solid #00aa00; border-radius: 8px; font-family: sans-serif;
                box-shadow: 0 0 25px rgba(0,255,0,0.2); display: flex; flex-direction: column;
                max-height: 90vh; transition: opacity 0.3s;
            `;
            ...
            closeBtn.onclick = () => {
                ...
                if (autoHideTimer) clearInterval(autoHideTimer); // 手动关闭也清除定时器
            };
        ...
        function addLinkToUI(data) {
            ...
            item.innerHTML = `
                ...
                <button class="copy-btn" style="width:100%; background:#222; color:#aaa; border:1px solid #444; padding:5px; cursor:pointer; font-size:12px; transition:all 0.2s;">复制链接</button>
            `;
            ...
            if (url.includes('/hls/') || desc.includes('子链接')) list.insertBefore(item, list.firstChild);
    ...
    function report(url, source, desc = '') {
        ...
        if (IS_TOP) setTimeout(() => addLinkToUI(data), 0);
        else window.top.postMessage({ type: 'M3U8_REPORT', data }, '*');
    }

    // 自动解析 Master Playlist 内容
